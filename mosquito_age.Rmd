...
```{r add_boxplot}
add_boxplot <- function(x, y, f = log10, t = 1e-20, eps = .03, lwd = 2, col = 1) {
  ...
  polygon(c(x[2], x[4], x[4], x[2]), rep(c(y - eps, y + eps), each = 2), lwd = lwd, border = col, col = adjustcolor(col, .2))
}
```
...
plot(NA, xlim = c(from, to), ylim = c(0, 2.5), xaxs = "i", yaxs = "i",
