...
```{r}
f <- function(d, x) {
  glm(matrix(c(broken_oocyst, oocyst), ncol = 2) ~ poly(total_oocyst, d, raw = TRUE),
    binomial, x)
}

f(2, filter(fig4b, age_class == "4-day-old"))

a <- map(1:20, f, filter(fig4b, age_class == "4-day-old"))
b <- map_dbl(a, AIC)
which.min(b)
plot(b[1:17])
```
...
```{r}
eps <- .15 # to prevent the 2 age classes to overlap
...
legend("topleft", legend = c("4-day-old", "12-day-old"), col = 1:2, lty = 1, bty = "n")
```
