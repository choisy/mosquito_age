...
```{r}
age4 <- fig3 |> 
  filter(age_class == "4-day-old", infection_status == "Exposed-infected") |> 
  pull(daysPI)
```
...
```{r}
mean(sim_age4)
mean(sim_age12_ExpInf)
mean(sim_age12_ExpNoinf)
mean(sim_age12_Uninf)
```
...
```{r}
prop_below_th(sim_age4)
prop_below_th(sim_age12_ExpInf)
prop_below_th(sim_age12_ExpNoinf)
prop_below_th(sim_age12_Uninf)
```
...
```{r}
mean(above_threshold(sim_age4))
mean(above_threshold(sim_age12_ExpInf))
mean(above_threshold(sim_age12_ExpNoinf))
mean(above_threshold(sim_age12_Uninf))
```
...
```{r}
stderror(above_threshold(sim_age4))
stderror(above_threshold(sim_age12_ExpInf))
stderror(above_threshold(sim_age12_ExpNoinf))
stderror(above_threshold(sim_age12_Uninf))
```
